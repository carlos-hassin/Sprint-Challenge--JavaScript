...
graduates.forEach(cont => {
  const contact = `${cont.first_name} ${cont.email}`;
...
nameUni = graduates.filter( name => name.university[0].includes("U") && name.university[1].includes("n") && name.university[2].includes("i"));
...
zooAnimals.forEach( scientificName => {
  let addName = `Name: ${scientificName.animal_name} Scientific: ${scientificName.scientific_name}`;
  displayNames.push(addName);
})
...
lowCaseAnimalNames = zooAnimals.map( lowerCap => lowerCap.animal_name.toLowerCase());
